                                ...
                                <div class="col-md-3 col-sm-12">
                                    <div class="card">
                                        <div class="card-body d-flex flex-column gap-1">
                                            ...
                                            <div class="mt-2">{{ $userAddress->address }}</div>
                                            <div class="mt-2">{{ $userAddress->locality }}</div>
                                            <div class="mt-2">
                                                {{ $userAddress->city }},
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            ...
                            <form action="{{ route('admin.orders.update', $orders->details[0]->order->id) }}"
                                method="POST">
                                @csrf
                                @method('PUT')
                                <table class="table header-border table-responsive-sm">
                                    ...
                                    <tbody>
                                        @foreach ($orders->details as $orderDetail)
                                            <tr>
                                                <td>{{ $loop->iteration }}</td>
                                                <td>
                                                    <input type="checkbox" name="selectedOrderIds[]" value="{{ $orderDetail->id }}">
                                                    <input type="hidden" name="hiddent_ids[{{ $orderDetail->id }}]" value="{{ $orderDetail->id }}">
                                                </td>                                                
                                                <td>
                                                    <select name="delivery_statuses[{{ $orderDetail->id }}]"
                                                        class="form-control">
                                                        <option value="pending"
                                                            {{ $orderDetail->status == 'pending' ? 'selected' : '' }}>
                                                            Pending</option>
                                                        <option value="processing"
                                                            {{ $orderDetail->status == 'processing' ? 'selected' : '' }}>
                                                            Processing</option>
                                                        <option value="completed"
                                                            {{ $orderDetail->status == 'completed' ? 'selected' : '' }}>
                                                            Completed</option>
                                                        <option value="cancelled"
                                                            {{ $orderDetail->status == 'cancelled' ? 'selected' : '' }}>
                                                            Cancelled</option>
                                                    </select>
                                                </td>
                                                <td>
                                                    <img src="{{ asset('storage/uploads/product/thumbnail/') }}/{{ $orderDetail->product->thumbnail }}"
                                                        style="width: 100px" alt="{{ $orderDetail->product->name }}">
                                                </td>
                                                <td>
                                                    {{ Str::limit($orderDetail->product->name, 50) }}
                                                </td>
                                                <td>
