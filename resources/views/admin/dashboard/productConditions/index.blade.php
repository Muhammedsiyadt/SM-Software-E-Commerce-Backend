            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">Manage Product Condition</h4>
                        <div class="d-grid gap-4 justify-content-between">
                            ...
                            <a href="{{ route('admin.productConditions.create') }}"
                                class="btn btn-sm btn-success text-white">Add Product Condition</a>
                        </div>
                    </div>
                    <div class="card-body">
                        <div class="card-body">
...
@section('scripts')
    <script>
        ...
        $.ajax({
            url: "{{ route('admin.productConditions.index') }}",
            method: "GET",
            dataType: "json",
        ...
        $(function() {
            ...
            var table = $('.product-datatable').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('admin.productConditions.index') }}",
                columnDefs: [{
                        targets: 1,
...

                    {
                        targets: 2,
                        render: function(data, type, row, meta) {
                            var editUrl = "{{ route('admin.productConditions.edit', ':id') }}"
                                .replace(':id', row.id);
                            var deleteUrl = "{{ route('admin.productConditions.destroy', ':id') }}"
                                .replace(':id', row.id);
