...
class OrderController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        $hiddentIds = $request->input('hiddent_ids', []);
        $deliveryStatuses = $request->input('delivery_statuses', []);
    
        // Iterate over the data and update order details
        foreach ($hiddentIds as $orderDetailId) {
            if (isset($deliveryStatuses[$orderDetailId])) {
                $status = $deliveryStatuses[$orderDetailId];
    
                // Find and update order detail by ID
                $orderDetail = OrderDetail::findOrFail($orderDetailId);
                $orderDetail->update(['status' => $status]);
            }
        }
    
        // Optionally, redirect back or return a response indicating success
        return redirect()->back()->with('success', 'Delivery statuses updated successfully');
    }
